...
def read_textures():
    '''Inputs all png files of nxn into a matrix in mean-deviation form'''
    textures = glob.glob(os.path.join(texture_folder+"/", '*.png'))
    all_textures=[]

    for file_path in textures:
        image = Image.open(file_path)
        if image.size == (n, n): #Throw out all not nxn images.
            image_gray = image.convert('RGBA').convert('L')
            matrix = np.array(image_gray)
...
def read_block():
    ...
    global w,w_last
    w_last=None
    w= Vh@(read_texture("textures/"+base_block+".png"))
    return w
...
def update_selected_block():
    ...
    try:
        ...
        messagebox.showerror("Block Not Found", base_block+".png was not found!")
        return
